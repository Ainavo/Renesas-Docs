...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="_5-3-2修改显示驱动接口文件-v1-1硬件" tabindex="-1"><a class="header-anchor" href="#_5-3-2修改显示驱动接口文件-v1-1硬件" aria-hidden="true">#</a> 5.3.2修改显示驱动接口文件(v1.1硬件)</h3><p>这里的修改是为了适配 v1.1 显示屏，如果你是用的不是 v1.1 显示屏，请阅读后面小节适配 v1.2 的内容。</p><p>打开 <code>03_dshanmcu_ra6m5_lvgl_display_touchpad\\dshanmcu_ra6m5\\drivers\\lv_port_disp.c</code> 文件，下面对其进行修改适配我们的工程：</p><ol><li>修改第 7 行为：</li></ol><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">if</span> <span class="token expression"><span class="token number">1</span></span></span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_5-3-3修改输入驱动接口文件-v1-1硬件" tabindex="-1"><a class="header-anchor" href="#_5-3-3修改输入驱动接口文件-v1-1硬件" aria-hidden="true">#</a> 5.3.3修改输入驱动接口文件(v1.1硬件)</h3><p>这里的修改是为了适配 gt911 触摸屏，如果你是用的不是 gt911 触摸屏，请阅读下一小节（v1.2）适配 ft5x06 的内容。</p><p>打开 <code>03_dshanmcu_ra6m5_lvgl_display_touchpad\\dshanmcu_ra6m5\\drivers\\lv_port_indev.c</code> 文件，下面对其进行修改适配我们的工程：</p><ol><li>修改第 7 行为：</li></ol><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">if</span> <span class="token expression"><span class="token number">1</span></span></span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_5-3-2修改显示驱动接口文件-v1-2硬件" tabindex="-1"><a class="header-anchor" href="#_5-3-2修改显示驱动接口文件-v1-2硬件" aria-hidden="true">#</a> 5.3.2修改显示驱动接口文件(v1.2硬件)</h3><p>这里的修改是为了适配 v1.2 显示屏，如果你是用的是不是 v1.2 显示屏，请阅读上一小节适配 v1.1 的内容。</p><p>打开 <code>03_dshanmcu_ra6m5_lvgl_display_touchpad\\dshanmcu_ra6m5\\drivers\\lv_port_disp.c</code> 文件，下面对其进行修改适配我们的工程：</p><ol><li>修改第 7 行为：</li></ol><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">if</span> <span class="token expression"><span class="token number">1</span></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ol start="2"><li>修改 12 行为：</li></ol><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">include</span> <span class="token string">&quot;lv_port_disp.h&quot;</span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ol start="3"><li>在 15 行空白处添加头文件包含：</li></ol><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">include</span> <span class="token string">&quot;drv_spi_display.h&quot;</span></span>
<span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">include</span> <span class="token string">&lt;stdio.h&gt;</span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><ol start="4"><li>在 20 行空白处添加下面两行代码：</li></ol><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">define</span> <span class="token macro-name">MY_DISP_HOR_RES</span>    <span class="token expression"><span class="token number">320</span></span></span>
...
<span class="token punctuation">{</span>
    ...
    <span class="token comment">/* Example for 1) */</span>
    <span class="token keyword">static</span> <span class="token class-name">lv_disp_draw_buf_t</span> draw_buf_dsc_1<span class="token punctuation">;</span>
    <span class="token comment">//static lv_color_t buf_1[MY_DISP_HOR_RES * 10];                          /*A buffer for 10 rows*/</span>
    <span class="token function">lv_disp_draw_buf_init</span><span class="token punctuation">(</span><span class="token operator">&amp;</span>draw_buf_dsc_1<span class="token punctuation">,</span> buf_1<span class="token punctuation">,</span> <span class="token constant">NULL</span><span class="token punctuation">,</span> MY_DISP_HOR_RES <span class="token operator">*</span> <span class="token number">10</span><span class="token punctuation">)</span><span class="token punctuation">;</span>   <span class="token comment">/*Initialize the display buffer*/</span>

    <span class="token comment">/* Example for 2) */</span>
    <span class="token comment">//static lv_disp_draw_buf_t draw_buf_dsc_2;</span>
    <span class="token comment">//static lv_color_t buf_2_1[MY_DISP_HOR_RES * 10];                        /*A buffer for 10 rows*/</span>
    <span class="token comment">//static lv_color_t buf_2_2[MY_DISP_HOR_RES * 10];                        /*An other buffer for 10 rows*/</span>
    <span class="token comment">//lv_disp_draw_buf_init(&amp;draw_buf_dsc_2, buf_2_1, buf_2_2, MY_DISP_HOR_RES * 10);   /*Initialize the display buffer*/</span>
...
<span class="token punctuation">{</span>
     <span class="token comment">/*You code here*/</span>
    ptDispDev <span class="token operator">=</span> <span class="token function">LCDGetDevice</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    ...
    <span class="token punctuation">{</span>
        <span class="token comment">//printf(&quot;Failed to get LCD device!\\r\\n&quot;);</span>
    ...
    <span class="token comment">//ptDispDev-&gt;SetDisplayWindow(ptDispDev, 0, 0, ptDispDev-&gt;wXres - 1, ptDispDev-&gt;wYres - 1);</span>
    <span class="token comment">/* 清除屏幕 */</span>
    <span class="token comment">//memset((uint8_t*)ptDispDev-&gt;FBBase, 0x00, ptDispDev-&gt;dwSize);</span>
    <span class="token comment">//ptDispDev-&gt;Flush(ptDispDev);</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><ol start="7"><li>修改 <code>disp_flush</code> 函数为如下代码：</li></ol><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">disp_flush</span><span class="token punctuation">(</span><span class="token class-name">lv_disp_drv_t</span> <span class="token operator">*</span> disp_drv<span class="token punctuation">,</span> <span class="token keyword">const</span> <span class="token class-name">lv_area_t</span> <span class="token operator">*</span> area<span class="token punctuation">,</span> <span class="token class-name">lv_color_t</span> <span class="token operator">*</span> color_p<span class="token punctuation">)</span>
<span class="token punctuation">{</span>
<span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">if</span> <span class="token expression"><span class="token number">1</span></span></span>
    <span class="token comment">//DisplayDevice *ptDispDev = LCDGetDevice();</span>
    <span class="token keyword">if</span><span class="token punctuation">(</span>disp_flush_enabled<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token comment">/*The most simple case (but also the slowest) to put all pixels to the screen one-by-one*/</span>

        <span class="token comment">//int32_t x;</span>
        <span class="token comment">//int32_t y;</span>
        <span class="token comment">//for(y = area-&gt;y1; y &lt;= area-&gt;y2; y++) {</span>
        <span class="token comment">//    for(x = area-&gt;x1; x &lt;= area-&gt;x2; x++) {</span>
        <span class="token comment">//        /*Put a pixel to the display. For example:*/</span>
        <span class="token comment">//        /*put_px(x, y, *color_p)*/</span>
        <span class="token comment">//        ptDispDev-&gt;SetPixel(ptDispDev, (unsigned short)x, (unsigned short)y, color_p);</span>
        <span class="token comment">//        color_p++;</span>
        <span class="token comment">//    }</span>
        <span class="token comment">//}</span>

        <span class="token class-name">uint32_t</span> size <span class="token operator">=</span> <span class="token function">lv_area_get_width</span><span class="token punctuation">(</span>area<span class="token punctuation">)</span> <span class="token operator">*</span> <span class="token function">lv_area_get_height</span><span class="token punctuation">(</span>area<span class="token punctuation">)</span><span class="token punctuation">;</span>
        ptDispDev<span class="token operator">-&gt;</span><span class="token function">SetDisplayWindow</span><span class="token punctuation">(</span>ptDispDev<span class="token punctuation">,</span> <span class="token punctuation">(</span><span class="token keyword">unsigned</span> <span class="token keyword">short</span><span class="token punctuation">)</span>area<span class="token operator">-&gt;</span>x1<span class="token punctuation">,</span> <span class="token punctuation">(</span><span class="token keyword">unsigned</span> <span class="token keyword">short</span><span class="token punctuation">)</span>area<span class="token operator">-&gt;</span>y1<span class="token punctuation">,</span> <span class="token punctuation">(</span><span class="token keyword">unsigned</span> <span class="token keyword">short</span><span class="token punctuation">)</span>area<span class="token operator">-&gt;</span>x2<span class="token punctuation">,</span> <span class="token punctuation">(</span><span class="token keyword">unsigned</span> <span class="token keyword">short</span><span class="token punctuation">)</span>area<span class="token operator">-&gt;</span>y2<span class="token punctuation">)</span><span class="token punctuation">;</span>
        ptDispDev<span class="token operator">-&gt;</span><span class="token function">Flush</span><span class="token punctuation">(</span>ptDispDev<span class="token punctuation">,</span> color_p<span class="token punctuation">,</span> size <span class="token operator">*</span> <span class="token number">2</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
<span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">endif</span></span>
    ...
    <span class="token function">lv_disp_flush_ready</span><span class="token punctuation">(</span>disp_drv<span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><ol start="7"><li>打开 <code>03_dshanmcu_ra6m5_lvgl_display_touchpad\\dshanmcu_ra6m5\\drivers\\lv_port_disp.h</code> 文件这里只需修改一个地方，修改第 7-10 行为：</li></ol><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">if</span> <span class="token expression"><span class="token number">1</span></span></span>

<span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">ifndef</span> <span class="token expression">LV_PORT_DISP_H</span></span>
<span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">define</span> <span class="token macro-name">LV_PORT_DISP_H</span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_5-3-3-修改输入驱动接口文件-v1-2硬件" tabindex="-1"><a class="header-anchor" href="#_5-3-3-修改输入驱动接口文件-v1-2硬件" aria-hidden="true">#</a> 5.3.3 修改输入驱动接口文件(v1.2硬件)</h3><p>这里的修改是为了适配 ft5x06 触摸屏，如果你是用的不是 ft5x06 触摸屏，请阅读上一小节(v1.1)适配 gt911 的内容。</p><p>打开 <code>03_dshanmcu_ra6m5_lvgl_display_touchpad\\dshanmcu_ra6m5\\drivers\\lv_port_indev.c</code> 文件，下面对其进行修改适配我们的工程：</p><ol><li>修改第 7 行为：</li></ol><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">if</span> <span class="token expression"><span class="token number">1</span></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ol start="2"><li>修改 12 行为：</li></ol><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">include</span> <span class="token string">&quot;lv_port_indev.h&quot;</span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ol start="3"><li>在 15 行空白处添加头文件包含：</li></ol><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">include</span> <span class="token string">&quot;drv_i2c_touchpad.h&quot;</span></span>
<span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">include</span> <span class="token string">&lt;stdio.h&gt;</span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><ol start="4"><li>修改 <code>lv_port_indev_init</code> 函数为如下代码：</li></ol><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token keyword">void</span> <span class="token function">lv_port_indev_init</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token punctuation">)</span>
...
<span class="token punctuation">{</span>
      <span class="token comment">/*Your code comes here*/</span>
    ptDev <span class="token operator">=</span> <span class="token function">TouchDevGet</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    ...
    <span class="token punctuation">{</span>
        <span class="token comment">//printf(&quot;Error. Not Found Touch Device!\\r\\n&quot;);</span>
        <span class="token keyword">return</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><img src="http://photos.100ask.net/renesas-docs/DShanMCU_RA6M5/lvgl_port_special_tutorial/chapter-5/chapter-5_014.png" alt="chapter-5_014" style="zoom:80%;"><p>按照上图配置之后再次点击编译，会发现编译警告都没有了，并且编译花费的时间也大大提高：</p><img src="http://photos.100ask.net/renesas-docs/DShanMCU_RA6M5/lvgl_port_special_tutorial/chapter-5/chapter-5_015.png" alt="chapter-5_015" style="zoom:80%;"><h2 id="_5-7-验证效果" tabindex="-1"><a class="header-anchor" href="#_5-7-验证效果" aria-hidden="true">#</a> 5.7 验证效果</h2><p>点击编译按钮，再点击 debug 按钮，将程序烧写到开发板中。会看到屏幕亮起一个漂亮的UI界面，并且可以通过点击触摸屏进行交互。</p>`,152),u=o("div",{STYLE:"page-break-after: always;"},null,-1);function v(m,k){const n=a("center");return e(),i("div",null,[r,t(n,null,{default:p(()=>[l("本节完")]),_:1}),u])}const b=s(d,[["render",v],["__file","chapter5.html.vue"]]);export{b as default};
